defmodule GameHubWeb.UserAuthTest do
  use GameHubWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias GameHub.Accounts
  alias GameHub.Accounts.Scope
  alias GameHubWeb.UserAuth

  import GameHub.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, GameHubWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      GameHubWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        GameHubWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
